...
public class CSVtoExcel {
    ...
    static void write(ArrayList<List<String>> array, Integer occurrences, String name, List<String> header)
        ...
        IntStream.range(0, occurrences).forEach(rowIndex -> {SXSSFRow row = sheet.createRow(rowIndex + 1); IntStream.range(0, array.get(rowIndex).size()).forEach(columnIndex -> row.createCell(columnIndex).setCellValue(array.get(rowIndex).get(columnIndex)));});
